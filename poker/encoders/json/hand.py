...
class HandEncoder(JSONEncoder):
    ...
    def default(self, object):
        ...
        if isinstance(object, Hand):
            ...
            json_repr["players"] = [p_encoder.default(x) for x in object.players]
            ...
            json_repr["our_cards"] = [c_encoder.default(x) for x in object.our_cards]
            json_repr["preflop"] = s_encoder.default(object.preflop)

            if object.first:
                json_repr["first"] = s_encoder.default(object.first)
            if object.second:
                json_repr["second"] = s_encoder.default(object.second)
            if object.third:
                json_repr["third"] = s_encoder.default(object.third)

            if object.flop:
                json_repr["flop"] = [c_encoder.default(x) for x in object.flop]
            if object.turn:
                json_repr["turn"] = [c_encoder.default(x) for x in object.turn]
            if object.river:
                json_repr["river"] = [c_encoder.default(x) for x in object.river]
