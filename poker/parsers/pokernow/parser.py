...
from poker.model import actions, admin
...
ADMIN_REGEX_STR = f'"The admin approved the player {PLAYER_REGEX_STR}"'
ADMIN_REGEX = re.compile(ADMIN_REGEX_STR)
...
def parse_game(game_data):
    ...
    in_between_lines = []
    last_end = 0
    for (start, end) in hand_line_ranges:
        for i in range(last_end, start):
            in_between_lines.append(lines[i])
        last_end = end + 1

    logging.debug("parsing Admin Lines")

    try:
        admin_actions = []

        for line in in_between_lines:
            parse_admin_action(line)

    except Exception as err:
        raise ValueError(f"couldn't parse admin action error: {err}")
    ...
    try:
        ...
        logging.debug("Parsing Game")
        logging.info(f"file of length:{len(lines)} was converted to {len(hands)}")
...
def parse_admin_action(line):
    """Parse the supplied line as an admin action"""

    admin_string, _, _ = line.rsplit(",", 2)
